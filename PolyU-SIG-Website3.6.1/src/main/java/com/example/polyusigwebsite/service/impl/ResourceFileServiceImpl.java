...
public class ResourceFileServiceImpl implements ResourceFileService {
    ...
    public void uploadBulk(BulkFolderUploadManifest manifest, List<MultipartFile> multipartFiles, String actor) throws IOException {
        ...
        // Track files per folder for audit logging
        Map<Folder, Integer> filesPerFolder = new HashMap<>();
        ...
        for (BulkFolderUploadManifest.FileNode fn : manifest.files()) {
            ...
            try {
                ...
                filesPerFolder.put(folder, filesPerFolder.getOrDefault(folder, 0) + 1);
            } catch (Exception ex) {
        ...
        // Create single audit log entry per folder with file count
        for (Map.Entry<Folder, Integer> entry : filesPerFolder.entrySet()) {
            Folder folder = entry.getKey();
            Integer fileCount = entry.getValue();
            String folderPath = buildFolderDisplayPath(folder);
            String message = String.format("Bulk uploaded %d file(s) to folder '%s'", fileCount, folderPath);
    ...
    public UploadSession initializeUploadSession(InitializeUploadRequest request, String actor) {
        ...
        auditService.record(AuditAction.UPLOAD_RESOURCE, actor,
                "Initialized upload session " + request.sessionId() + " for " + request.totalFiles() + " files");
    ...
    public ResourceFileResponse completeFileUpload(Long taskId, String actor) throws IOException {
        ...
        auditService.record(AuditAction.UPLOAD_RESOURCE, actor,
                "Completed upload task #" + taskId + " -> resource #" + saved.getId());
    ...
    public void completeUploadSession(String sessionId, String actor) {
        ...
        uploadQueueManager.resumeSession(sessionId);
        auditService.record(AuditAction.UPLOAD_RESOURCE, actor, "Started async processing for upload session " + sessionId);
