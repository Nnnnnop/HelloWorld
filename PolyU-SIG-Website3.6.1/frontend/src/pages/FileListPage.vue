  ...
  <div class="page">
        ...
        <el-card class="panel-card">
          ...
          >
            ...
            <el-table-column label="Type" width="80">
              <template #default="{ row }">
                {{ row.isFolder ? 'folder' : getFileType(row.title) }}
              </template>
            </el-table-column>
            ...
            <el-table-column prop="visibility" label="Access" width="90" align="center">
              <template #default="{ row }">
    ...
    <el-dialog v-model="infoVisible" title="File Info" width="600px">
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="File Size">{{ infoFile?.fileSize != null ? formatFileSize(infoFile.fileSize) : '-' }}</el-descriptions-item>
    ...
    <el-dialog v-model="folderInfoVisible" title="Folder Info" width="500px">
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="Access Level">
          {{ visibilityLabel(folderInfo?.visibility) }}
        </el-descriptions-item>
...
const openFolderInfo = (folder) => {
  const folderId = folder.isFolder ? String(folder.id) : folder.id
  const stats = calculateFolderStats(folderId)
  folderInfo.value = {
    id: folderId,
    name: folder.name || folder.title || 'Root',
...
const openFolderEdit = (folder) => {
  ...
  folderEditing.value = {
    ...
    visibility: folder.visibility || 'HIDDEN'
  }
...
const getDownloadIdsForRows = (rows) => {
  ...
  for (const row of rows || []) {
    ...
    if (row.isFolder) {
      const folderIds = new Set(getDescendantFolderIds(row.id))
      for (const file of files.value) {
        const fileFolderId = file.folderId == null ? '1' : String(file.folderId)
...
const downloadRowsAsZip = async (rows, zipName = 'selected-files.zip') => {
  ...
  try {
    ...
    a.href = url
    a.download = zipName.replace(/\s+/g, '_')
...
const onDownload = async (row) => {
  ...
  if (row.isFolder) {
    ...
    try {
      ...
      const a = document.createElement('a')
      a.href = url
      const sanitizedName = (row.title || 'folder').replace(/\s+/g, '_')
...
const handleRowAction = (row, command) => {
  if (command === 'info' && !row.isFolder) {
    ...
    if (row.isFolder) {
      openFolderEdit({ id: row.id, name: row.title, parentId: row.parentId ?? row.folderId ?? null })
    } else {
      openEdit(row)
    }
...
const initializeTableColumnResizing = (tableRef) => {
  ...
  headerCells.forEach((th, index) => {
    ...
    const minWidth = Math.max(60, Math.ceil(headerWidth + 20)) // Add some padding
