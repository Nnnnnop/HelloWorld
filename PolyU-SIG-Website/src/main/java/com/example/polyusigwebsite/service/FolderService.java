...
public class FolderService {
    ...
    public FolderDto updateFolder(Long id, String name, Long parentId, ResourceVisibility visibility) {
        ...
        if (visibility != null) {
            visibilityChanged = !visibility.equals(folder.getVisibility());
        ...
        if (visibilityChanged) {
            propagateVisibilityToDescendants(saved, visibility);
        }
    ...
    private void propagateVisibilityToDescendants(Folder folder, ResourceVisibility visibility) {
        for (Folder child : folderRepository.findByParentId(folder.getId())) {
            child.setVisibility(visibility);
            Folder savedChild = folderRepository.save(child);
            propagateVisibilityToDescendants(savedChild, visibility);
        }

        List<ResourceFile> files = resourceFileRepository.findByFolderId(folder.getId());
        if (!files.isEmpty()) {
