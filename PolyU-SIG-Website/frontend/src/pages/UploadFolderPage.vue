<template>
  <div class="page">
    ...
    <template #header>
      <div class="header-row">
        <div class="header-left-group">
          ...
          <el-space v-if="fileRows.length" size="small">
            <el-button size="small" @click="openChangeDestinationDialog">Change destination</el-button>
            <el-button size="small" :type="excludeTopLevelRootFolder ? 'primary' : 'default'" @click="excludeTopLevelRootFolder = !excludeTopLevelRootFolder">
              Exclude top level root folder
            </el-button>
            <span v-if="destinationFolderId" class="destination-text">📁 {{ destinationFolderPath || 'Root' }}</span>
          </el-space>
        </div>
        <el-space>
    ...
    <el-row v-if="fileRows.length || folderTree.length" :gutter="16" class="content">
      <el-col v-if="!isNarrow || showFolderPanel" :span="5">
        <el-card class="inner">
          ...
          <div class="folder-container">
            <el-tree
              :data="previewFolderTree"
              node-key="tempId"
              :default-expand-all="expandFolderTreeByDefault"
              :expand-on-click-node="false"
              :props="{ label: 'name', children: 'children' }"
              :current-node-key="selectedPreviewFolderId"
              highlight-current
    ...
    <div v-if="!fileRows.length && !folderTree.length" class="empty-state">
    ...
    <el-dialog v-model="destinationFolderDialogVisible" title="Select upload destination folder" width="600px">
      <div class="folder-selection-dialog">
        <el-input
          v-model="destinationFolderSearch"
          size="small"
          clearable
          placeholder="Search folder"
        ...
        <div class="folder-tree-container">
          <el-tree
            :data="filteredDestinationFolders"
            node-key="id"
            :props="{ label: 'name', children: 'children' }"
            :current-node-key="selectedDestinationFolderId"
            highlight-current
            :expand-on-click-node="false"
            @node-click="onSelectDestinationFolder"
          >
            <template #default="{ data }">
              <span class="tree-row">
      ...
      <template #footer>
        <el-button @click="destinationFolderDialogVisible = false">Cancel</el-button>
        <el-button type="primary" @click="confirmDestinationFolder">Confirm</el-button>
      </template>
    </el-dialog>
    </el-card>
  </div>
...
import { createFolder, listFolders } from '../api/folderApi'
...
const excludeTopLevelRootFolder = ref(false)
const destinationFolderId = ref(null)
const destinationFolderDialogVisible = ref(false)
const destinationFolderSearch = ref('')
const selectedDestinationFolderId = ref(null)
...
const previewFolderTree = computed(() => {
  if (folderTree.value.length) {
    // Filter out duplicate ROOT nodes from the tree
    return folderTree.value.filter(node => node.tempId !== 'ROOT')
  }
  return [
    {
      tempId: 'ROOT',
...
const destinationFolderPath = computed(() => {
  if (!destinationFolderId.value) return 'Root'

  const findFolderPath = (folderId, tree) => {
    for (const node of tree || []) {
      if (String(node.id) === String(folderId)) {
        return node.name
      }
      if (node.children && node.children.length) {
        const childPath = findFolderPath(folderId, node.children)
        if (childPath) {
  ...
  return findFolderPath(destinationFolderId.value, destinationFolderTree.value) || 'Root'
})

const dbFolderTree = ref([])
const destinationFolderTree = computed(() => {
  if (!dbFolderTree.value.length) {
    return [
      {
        id: 'ROOT',
        name: 'Root',
  ...
  const hasRootNode = dbFolderTree.value.some((node) => node.parentId == null || node.id === 1)
  return hasRootNode ? dbFolderTree.value : [
    {
      id: 'ROOT',
      name: 'Root',
      children: dbFolderTree.value
    }
  ]
})

const filteredDestinationFolders = computed(() => {
  const kw = String(destinationFolderSearch.value || '').trim().toLowerCase()
  const source = destinationFolderTree.value
  if (!kw) return source
...
const loadDestinationFolders = async () => {
  if (dbFolderTree.value.length) return
  try {
    dbFolderTree.value = await listFolders()
  } catch (error) {
    ElMessage.error(error.message || 'Failed to load folder directory')
  }
}
...
const openChangeDestinationDialog = async () => {
  await loadDestinationFolders()
  if (destinationFolderId.value) {
    selectedDestinationFolderId.value = destinationFolderId.value
  } else {
    const rootNode = destinationFolderTree.value.find((node) => node.parentId == null || node.id === 1) || destinationFolderTree.value[0]
    selectedDestinationFolderId.value = rootNode?.id ?? null
  }
  destinationFolderSearch.value = ''
  destinationFolderDialogVisible.value = true
}

const onSelectDestinationFolder = (node) => {
  if (!node) return
  selectedDestinationFolderId.value = node.id
}

const confirmDestinationFolder = () => {
  destinationFolderId.value = selectedDestinationFolderId.value
  destinationFolderDialogVisible.value = false
}
...
const buildUploadEntries = (files, excludeRoot = false) => {
  return Array.from(files).map((fileObj) => {
    const rel = normalizePath(fileObj.webkitRelativePath || fileObj.name)
    const parts = splitPath(rel)
    const clientPath = excludeRoot && parts.length > 1 ? parts.slice(1).join('/') : rel
    return { fileObj, clientPath }
  })
}

const rebuildUploadPreview = () => {
  if (!pickedFiles.value.length) return
  const entries = buildUploadEntries(pickedFiles.value, excludeTopLevelRootFolder.value)
...
const onPickDirectory = (e) => {
  ...
  const entries = buildUploadEntries(list, excludeTopLevelRootFolder.value)
  buildFolderTree(entries)
  buildFileRows(entries)
  for (const node of folderTree.value) applyInheritanceFrom(node)
}

watch(excludeTopLevelRootFolder, () => {
  if (!pickedFiles.value.length) return
  rebuildUploadPreview()
})
